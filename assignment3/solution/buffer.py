...
class PrioritizedReplayBuffer:
    def __init__(self, env, buffer_size, eps=1e-2, alpha=0.1, beta=0.1):
        ...
        self.state = torch.empty(buffer_size, env.observation_space._shape[0], dtype=torch.float)
        self.action = torch.empty(buffer_size, dtype=torch.float)
        self.reward = torch.empty(buffer_size, dtype=torch.float)
        self.next_state = torch.empty(buffer_size, env.observation_space._shape[0], dtype=torch.float)
