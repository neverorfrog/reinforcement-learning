...
class ReplayBuffer:
    def __init__(self, env_params, replay_strategy = 'future', replay_k = 4, reward_function = None, 
                 capacity = 1e6, alpha = 0.6, beta = 0.4, alpha_prime = 0.6, beta_prime = 0.4):
    ...
    def sample(self, batch_size=256):
        ...
        max_weight_ep = (min_prob * self.size_ep) ** (-self.beta) #max w_i for weighted importance sampling
        probs_ep = self.tree.sumtree[ep_indices + self.capacity_ep - 1] / self.tree.sum
        weights_ep = ((probs_ep * self.size_ep) ** (-self.beta)) / max_weight_ep
