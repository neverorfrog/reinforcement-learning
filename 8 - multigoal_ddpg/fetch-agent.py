...
class FetchAgent(Parameters):
    def __init__(self, name, env: gym.Env, board: ProgressBoard = None, window = 500, gamma = 0.98, prioritized = True,
                 polyak = 0.95, pi_lr = 0.0005, q_lr = 0.0005, eps = 0.3, noise_eps = 0.2, learning_rate = 1, logging_rate = 1,
                 action_l2 = 1., batch_size = 128, gradient_steps=50, success_threshold = 0.98, max_episodes=500):
...
def launch(env_name = 'FetchReach-v2', mode = None, prioritized = True):
    if mode == Mode.TRAIN:
        ...
        for seed in seeds:
            ...
            if prioritized:
                agent = FetchAgent(f"HGR_{env_name}_{seed}", env, max_episodes = 1000, window = 50)
            else:
                agent = FetchAgent(f"HER_{env_name}_{seed}", env, max_episodes = 1000, window = 50)
    ...
    if mode == Mode.TEST:
        ...
        if prioritized:
            agent = FetchAgent(f"HGR_{env_name}_{0}", env)
        else:
            agent = FetchAgent(f"HER_{env_name}_{0}", env)
        agent.load()
        agent.plot_success()
        for _ in range(10):
